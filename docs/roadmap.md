...
- **Puzzle variety** — 3 puzzle types rotate fast. Add 2-3 more (riddle/trivia about species lore, tile-sliding, affinity-matching minigame) to keep dungeon rooms fresh.
...
- **Tutorial completeness** — GDD 12.1 specifies guided overlays; verify all tutorial elements present
- **Formation UX** — strategic impact of front/back row isn't communicated well to new players
- **Text overflow** — various popups and labels may clip on different resolutions
- **Difficulty curve validation** — play through full 7-rift arc and verify pacing, resource pressure, enemy scaling
- **Balance pass** — technique power, status durations, capture rates, energy economy
- **Rift re-entry variation** — GDD says boss reshuffles content; verify it feels different on repeat
- **Save slot management** — currently single save; multiple slots or "are you sure?" on new game
...
- **Item icons** — text lists only; no visual representation
